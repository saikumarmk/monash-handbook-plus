    ...
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      {aosList.length === 0 && query && (
        <div className="text-center py-16">
          <p className="text-gray-400">No areas of study found matching "{query}"</p>
        </div>
      )}
    </div>
  )
...
function AOSDetail({ aos, onBack }: { aos: AreaOfStudy; onBack: () => void }) {
  ...
  // Calculate progress
  const completedUnits = Object.keys(aos.all_units).filter(code => hasUnit(code)).length
  const totalUnits = Object.keys(aos.all_units).length
